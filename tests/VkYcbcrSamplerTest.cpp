...
DEF_GPUTEST_FOR_VULKAN_CONTEXT(VkYCbcrSampler_DrawImageWithYcbcrSampler, reporter, context_info) {
...
DEF_GPUTEST_FOR_VULKAN_CONTEXT(VkYCbcrSampler_NoYcbcrSurface, reporter, context_info) {
    VkTestHelper testHelper(false);
    if (!testHelper.init()) {
        ERRORF(reporter, "VkTestHelper initialization failed.");
        return;
